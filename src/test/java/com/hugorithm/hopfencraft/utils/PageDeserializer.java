        ...
        if (p.isExpectedStartObjectToken()) {
            ...
            if (p.hasTokenId(JsonTokenId.ID_FIELD_NAME)) {
                ...
                do {
                    p.nextToken();
                    switch (propName) {
                        case CONTENT -> list = ctxt.readValue(p, valuesListType);
                        case NUMBER -> pageNumber = ctxt.readValue(p, Integer.class);
                        case SIZE -> pageSize = ctxt.readValue(p, Integer.class);
                        case TOTAL_ELEMENTS -> total = ctxt.readValue(p, Long.class);
